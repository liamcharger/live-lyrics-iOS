...
struct UserService {
    ...
    func updateSettings(_ user: User, wordCount: Bool, songSubtitle: String, wordCountStyle: String, showsExplicitSongs: Bool, metronomeStyle: [String], completion: @escaping(Bool, String) -> Void) {
        Firestore.firestore().collection("users").document(user.id!)
            .updateData(["wordCount": wordCount, "showDataUnderSong": songSubtitle, "wordCountStyle": wordCountStyle, "showsExplicitSongs": showsExplicitSongs, "metronomeStyle": metronomeStyle]) {
                error in
                if let error = error {
