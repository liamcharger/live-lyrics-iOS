            ...
            ScrollView {
                VStack {
                    if storeKitManager.purchasedProducts.isEmpty {
                        AdBannerView(unitId: "ca-app-pub-5671219068273297/7049802118", height: 50)
                            .padding(.bottom, 10)
                    }
                    if !mainViewModel.isLoadingRecentlyDeletedSongs {
                        ...
                        ForEach(searchableSongs, id: \.id) { song in
                            if song.title == "noSongs" {
                                ...
                                NavigationLink(destination: SongDetailView(song: songData, songs: nil, restoreSong: song, wordCountStyle: authViewModel.currentUser?.wordCountStyle ?? "Words", isDefaultSong: false, albumData: nil, folder: nil), label: {
